 ...
 *
 * The renderer pulls resume text + JD + the four setup options + a recent-answers
 ...
 *   - resume + JD are truncated to keep total input well under typical 8K context
...
export function renderGenerateQuestion(
  ...
  // one full round on this same resume/JD. We instruct the model to ramp difficulty
  // and reference patterns from prior answers so the experience feels like a real
  // follow-up interview, not a reset.
  const roundFraming =
    round > 1
      ? [
          `This is round ${round} of an ongoing interview. The candidate has already completed ${round - 1} full round(s) on this same resume and JD.`,
  ...
  const userParts = [
    ...
    truncate(ctx.resumeContent, 4000) || "(no resume provided)",
    "",
  ...
  // Per-resume question history — strict "do not repeat" guard. Pulled from the BE
  ...
  if (pastQuestions.length > 0) {
    ...
    userParts.push(
      "",
      `IMPORTANT — questions already asked of this candidate on this resume (DO NOT repeat or paraphrase any of these; pick a fresh angle):`,
  ...
  userParts.push(
    "",
    "Now produce the next question. Tailor it to the resume and the JD; if the candidate",
